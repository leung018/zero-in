...
5. Update existing test cases to make timer-based rules explicit (set `pauseBlockingWhenTimerNotRunning=false` and `pauseBlockingDuringBreaks=false` in the unchanged schedule-behavior path) as a regression lock, then refactor internals to keep logic readable and deterministic.
